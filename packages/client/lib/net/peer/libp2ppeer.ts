...
export class Libp2pPeer extends Peer {
  ...
  constructor(options: Libp2pPeerOptions) {
    ...
    const address = multiaddrs.map((ma) => ma.toString().split('/ipfs')[0]).join(',')
   ...
   * @return {Promise}
   */
  async accept(protocol: Protocol, connection: Connection, server: Libp2pServer): Promise<void> {
  ...
  async bindProtocols(
    ...
    await Promise.all(
      this.protocols.map(async (p) => {
        ...
        try {
          const conn = await node.dialProtocol(peer, protocol)
